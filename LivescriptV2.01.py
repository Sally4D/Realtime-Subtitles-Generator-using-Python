...
import zipfile
import subprocess
# New import for translation backends
from translator import get_translator, MarianTranslator, get_available_argos_languages
...
class SettingsManager:
    ...
    def __init__(self):
        ...
        self.default_settings = {
            ...
            "block_size": 3000,
            "model_path": LANGUAGE_MODELS["English"],
            "window_width": 1200,
            "window_height": 70,
            "window_padding": 20,
            "delay_threshold": 3.0,
            ...
            # Simplified translation settings
...
class AboutWindow(ctk.CTkToplevel):
    ...
    def __init__(self, parent):
        ...
        about_text = (
            "This application provides real-time transcription and optional translation of your system's audio output, "
...
class SettingsWindow(ctk.CTkToplevel):
    ...
    def __init__(self, parent, settings_manager, caption_window, restart_callback):
        ...
        self.argos_languages = get_available_argos_languages()
        self.marian_languages = sorted(list(MarianTranslator.LANG_CODE_MAP.keys()))
    ...
    def setup_ui(self):
        ...
        self.bg_color_btn = ctk.CTkButton(appearance_frame, text="", command=self.pick_bg_color, border_width=1,
        ...
        ctk.CTkLabel(audio_frame, text="Audio & Transcription", font=ctk.CTkFont(weight="bold")).grid(row=0, column=0,
                                                                                                      columnspan=3,
                                                                                                      pady=(5, 10))
        language_label = ctk.CTkLabel(audio_frame, text="Spoken Language:")
        ...
        # --- Translation Section ---
        self.translation_frame = ctk.CTkFrame(scroll_frame)
        self.translation_frame.grid(row=3, column=0, columnspan=2, padx=10, pady=10, sticky="ew")
        self.translation_frame.grid_columnconfigure(1, weight=1)
        ctk.CTkLabel(self.translation_frame, text="Translation", font=ctk.CTkFont(weight="bold")).grid(row=0, column=0,
                                                                                                       columnspan=3,
                                                                                                       pady=(5, 10))
        enable_translation_label = ctk.CTkLabel(self.translation_frame, text="Enable Translation:")
        enable_translation_label.grid(row=1, column=0, padx=10, pady=5, sticky="w")
        self.enable_translation_checkbox = ctk.CTkCheckBox(self.translation_frame, text="",
                                                           command=self.on_translation_toggle)
        self.enable_translation_checkbox.grid(row=1, column=1, padx=10, pady=5, sticky="w")

        backend_label = ctk.CTkLabel(self.translation_frame, text="Translation Engine:")
        backend_label.grid(row=2, column=0, padx=10, pady=5, sticky="w")
        self.backend_menu = ctk.CTkOptionMenu(self.translation_frame, values=["ArgosTranslate", "MarianMT"],
                                              command=self.on_backend_change)
        self.backend_menu.grid(row=2, column=1, columnspan=2, padx=10, pady=5, sticky="ew")

        self.target_language_label = ctk.CTkLabel(self.translation_frame, text="Translate To:")
        self.target_language_label.grid(row=3, column=0, padx=10, pady=5, sticky="w")
        self.target_language_menu = SearchableComboBox(self.translation_frame, values=[],
                                                       command=self.on_target_language_change)
        self.target_language_menu.grid(row=3, column=1, columnspan=2, padx=10, pady=5, sticky="ew")

        self.translation_model_status_label = ctk.CTkLabel(self.translation_frame, text="", text_color="gray")
        self.translation_model_status_label.grid(row=4, column=0, columnspan=3, pady=5)
        self.translation_download_progress = ctk.CTkProgressBar(self.translation_frame, orientation="horizontal")
        self.translation_download_progress.set(0)

        # --- Restart Prompt Section ---
        ...
        self.restart_frame.grid_remove()
    ...
    def on_backend_change(self, backend):
        if backend == "MarianMT":
            self.target_language_menu.configure(values=self.marian_languages)
            if self.target_language_menu.get() not in self.marian_languages:
                self.target_language_menu.set(
                    "Spanish" if "Spanish" in self.marian_languages else self.marian_languages[0])
        else:  # ArgosTranslate
            self.target_language_menu.configure(values=self.argos_languages)
            if self.target_language_menu.get() not in self.argos_languages:
                self.target_language_menu.set(
                    "Spanish" if "Spanish" in self.argos_languages else self.argos_languages[0])

        self.check_translation_model_status()
        self.update_setting()
    ...
    def load_settings_to_ui(self):
        ...
        self.show_about_checkbox.select() if self.settings[
            'show_about_on_startup'] else self.show_about_checkbox.deselect()
        self.enable_translation_checkbox.select() if self.settings[
            'translation_enabled'] else self.enable_translation_checkbox.deselect()
        self.backend_menu.set(self.settings['translation_backend'])
        self.target_language_menu.set(self.settings['translation_target_language'])

        self.on_translation_toggle()
        self.on_backend_change(self.settings['translation_backend'])
    ...
    def update_setting(self, value=None):
        ...
        self.settings['translation_enabled'] = bool(self.enable_translation_checkbox.get())
        self.settings['translation_target_language'] = self.target_language_menu.get()
        self.settings['translation_backend'] = self.backend_menu.get()
    ...
    def on_translation_toggle(self):
        is_enabled = bool(self.enable_translation_checkbox.get())
        state = "normal" if is_enabled else "disabled"

        self.backend_menu.configure(state=state)
        self.target_language_label.configure(state=state)
        self.target_language_menu.configure(state=state)

        if is_enabled:
            self.on_backend_change(self.backend_menu.get())
        else:
            self.translation_model_status_label.configure(text="")
    ...
    def check_model_status(self):
        ...
        if model_path and os.path.isdir(model_path):
            self.model_status_label.configure(text=f"Vosk model '{model_path}' found.", text_color="green")
        else:
            self.model_status_label.configure(text=f"Vosk model for {selected_lang} not found. Downloading...",
                                              text_color="orange")
            self.download_and_extract_model(model_path)

    def download_and_extract_model(self, model_name):
        if not model_name:
            self.model_status_label.configure(text="Invalid Vosk model name.", text_color="red")
            return
    ...
    def _download_worker(self, model_name):
        ...
        try:
            ...
            with zipfile.ZipFile(zip_path, 'r') as zip_ref:
                zip_ref.extractall()
            os.remove(zip_path)
            self.model_status_label.configure(text=f"Vosk model '{model_name}' installed.", text_color="green")
            self.show_restart_prompt()
        except Exception as e:
            self.model_status_label.configure(text=f"Error downloading Vosk model: {e}", text_color="red")
    ...
    def check_translation_model_status(self):
        if not self.settings['translation_enabled']:
            self.translation_model_status_label.configure(text="Translation disabled.", text_color="gray")
            return

        def status_callback(message, color):
            self.translation_model_status_label.configure(text=message, text_color=color)

        translator = get_translator(self.settings, status_callback)

        if not translator.is_ready:
            download_thread = threading.Thread(target=self.download_translation_model, daemon=True)
            download_thread.start()
    ...
    def download_translation_model(self):
        ...
        self.save_button.configure(state="disabled")
        self.translation_download_progress.grid(row=5, column=0, columnspan=3, padx=10, pady=5, sticky="ew")
        self.translation_download_progress.configure(mode="indeterminate")
        self.translation_download_progress.start()

        from_lang_name = self.language_menu.get().split(" ")[0]
        to_lang_name = self.target_language_menu.get()

        try:
            if backend == "ArgosTranslate":
                self.translation_model_status_label.configure(
                    text=f"Downloading Argos model for {from_lang_name} -> {to_lang_name}...")
                argostranslate.package.update_package_index()
                available_packages = argostranslate.package.get_available_packages()

                package_to_install = next(
                    filter(
                        lambda x: x.from_name == from_lang_name and x.to_name == to_lang_name,
                        available_packages,
                    )
                )
                argostranslate.package.install_from_path(package_to_install.download())
                self.translation_model_status_label.configure(text=f"Argos model installed.", text_color="green")

            elif backend == "MarianMT":
                model_name = f'Helsinki-NLP/opus-mt-{MarianTranslator.LANG_CODE_MAP[from_lang_name]}-{MarianTranslator.LANG_CODE_MAP[to_lang_name]}'
                self.translation_model_status_label.configure(text=f"Downloading MarianMT model: {model_name}...")
            ...
            self.translation_model_status_label.configure(
                text=f"No direct translation model found for {from_lang_name} -> {to_lang_name}", text_color="orange")
        except Exception as e:
            self.translation_model_status_label.configure(text=f"Error downloading model: {e}", text_color="red")
        finally:
            self.translation_download_progress.stop()
            self.translation_download_progress.grid_forget()
...
def speech_recognition_thread(settings, sample_rate):
    """Processes audio from the queue using Vosk and optionally translates it."""
    global last_caption_time
    ...
    try:
        ...
        caption_queue.put(f"ERROR: Failed to load Vosk model. Please check settings.")
        return

    translator = get_translator(settings)
    if translator:
        print(f"Translation enabled with backend: {settings.get('translation_backend')}")
    else:
    ...
    while not stop_threads.is_set():
        try:
            ...
            if recognizer.AcceptWaveform(audio_data):
                ...
                if text:
                    if translator and translator.is_ready:
                ...
                if partial_text:
                    # **MODIFIED**: Translate partial text for real-time effect
                    if translator and translator.is_ready:
                        partial_text = translator.translate(partial_text)
...
def main():
    ...
    recognizer_thread = threading.Thread(target=speech_recognition_thread, args=(settings, SAMPLE_RATE), daemon=True)
    ...
    if is_first_run or settings.get('show_about_on_startup', True):
        root.after(100, app.open_about_window)
